...
timeout -k 5s 7200s python3 paralela.py -a debug=$DEBUG &> "logs/log_ev_i_rj${start_datetime}.txt"

timeout -k 5s 7200s scrapy crawl rog_joma -a debug=$DEBUG &> "logs/log_rj${start_datetime}.txt" && echo OK || echo Failed, status $?

timeout -k 5s 7200s scrapy crawl keindl_sport -a debug=$DEBUG &> "logs/log_ks${start_datetime}.txt" && echo OK || echo Failed, status $?
