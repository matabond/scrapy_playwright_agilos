...
class BikeSpider2(scrapy.Spider):
    ...
    async def parse(self, response):
        ...
        await page.close()
        extracted_links=set()
        for link in response.css('div.sub-navigation li.navigation--entry'):    # dohvati mi sve li.navigation--entry unutar div.sub-nav....
    ...
    async def parse_site(self, response):
       ...
       # extracted_links_site=set()
        for link in response.css('div.product--info'):    # dohvati mi sve li.navigation--entry unutar div.sub-nav....
            extracted_links_site.add(link.css('a::attr(href)').get())
        print('linkovi:', extracted_links_site, 'duljina:', len(extracted_links_site))
        next_page = response.css('div.listing--bottom-paging a[title*=Next]').attrib['href']  #ovo bi sad trebalo bit ok ali za zadnjem ne može uzet next pa je tu greška, to riješi
